...
where x is the total number of seconds to wait before timing out.\n"""
        ...
        while (time.time() - start_time < read_timeout) or (not read_timeout):
            ...
            # 对一些设备的特殊处理
            self._handle_device_patterns_from_config(output, re_flags=re_flags)
...
You can also look at the Netmiko session_log or debug log for more information.\n\n"""
    ...
    def _handle_device_patterns_from_config(self, output, re_flags=0):
        """
        从 netmiko/device_patterns.json 读取配置，根据 device_type 匹配 pattern，命中则自动发送 send_content。
        同一个 pattern 只会执行一次（每个连接周期内）。
        """
        config_path = path.join("device_patterns.json")
        ...
        try:
            ...
            return
        device_type = self.device_type.replace("_telnet", "").replace("_ssh", "")
        patterns = config.get(device_type, [])

        # 初始化 pattern 命中记录
        if not hasattr(self, "_pattern_matched_set"):
            self._pattern_matched_set = set()

        for item in patterns:
            pattern = item.get("pattern")
            send_content = item.get("send_content")
            # 用 pattern 字符串做唯一标识
            if pattern and send_content:
                if pattern in self._pattern_matched_set:
                    continue
                if re.search(pattern, output, flags=re_flags):
                    log.info(f"[pattern config] Detected pattern '{pattern}', sent '{send_content}'")
                    self.write_channel(send_content)
                    self.write_channel(self.RETURN)
                    self._pattern_matched_set.add(pattern)
                    # 若只需匹配一次，可 break
                    break
